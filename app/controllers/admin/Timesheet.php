...
class Timesheet extends MY_Controller {
	...
	public function import_attendance() {
		...
		if ($this->input->post('is_ajax') == '3') {
			...
			$csvMimes = array('application/vnd.ms-excel', 'application/msexcel', 'application/x-msexcel', 'application/x-ms-excel', 'application/x-excel', 'application/x-dos_ms_excel', 'application/xls', 'application/x-xls', 'application/excel', 'application/download', 'application/vnd.ms-office', 'application/msword');
			...
			if (empty($_FILES['file']['name'])) {
				...
				if ($_FILES['file']['type']) {
					if (is_uploaded_file($_FILES['file']['tmp_name'])) {
						...
						// check file size
						if (filesize($_FILES['file']['tmp_name']) > 51200000) {
							...
							foreach ($sheetData as $key) {
								...
								//$in=$this->Xin_model->ambilin($key['B'],$user[0]->user_id);
								$ambil_shift = $this->Xin_model->ambilshift($user[0]->user_id, $key['B']);
								$id_shift = ($ambil_shift == true) ? $ambil_shift : "NOT SET";

								$office_shift = $this->Timesheet_model->shift_import($id_shift, $key['B']);

								$attendance_date = $key['B'];
								$clock_in = $key['C'];
								$clock_out = $key['D'];
								...
								if (!is_null($user)) {
									$id_absensi = $user[0]->id_absensi;
								} else {
									$id_absensi = null;
								}
								$h_date_chck = $this->Timesheet_model->cek_libur($key['B']);
								$holiday_arr = array();
								if ($h_date_chck->num_rows() == 1) {
									$h_date = $this->Timesheet_model->ambil_libur($key['B']);
								...
								$leave_date_chck = $this->Timesheet_model->leave_date_check($user[0]->user_id, $key['B']);
								$leave_arr = array();
								if ($leave_date_chck->num_rows() > 0) {
									$leave_date = $this->Timesheet_model->leave_date($user[0]->user_id, $key['B']);
								...
								if ($day == "Saturday" or $day == "Sunday") {
									...
									if (!empty($key['C']) and !empty($key['D'])) {
										$status = "Overtime Holidays";
									}
								} elseif (in_array($key['B'], $leave_arr) == 1) {

									$status = "Cuti";

								} elseif (in_array($attendance_date, $holiday_arr) == 1) {
									$status = "Libur";
								...
								$cek = $this->Timesheet_model->cekabsen($user[0]->user_id, $attendance_date);
	...
	public function attendance_list() {
		...
		foreach ($employee->result() as $r) {
			...
			// office shift
			$office_shift = $this->Timesheet_model->shift_employe($r->office_shift_id);
			...
			if ($check->num_rows() > 0) {
				...
				if ($office_shift[0]->monday_in_time == '' && $day == 'Monday') {
					...
					$status = $this->lang->line('xin_holiday');
				} else if (in_array($attendance_date, $holiday_arr)) {
					// holiday
					$status = $this->lang->line('xin_holiday');
					...
					$status = $this->Timesheet_model->get_shift_employee($r->employee_id, $attendance_date);
	...
	public function updateabsen() {
		...
		foreach ($date_range as $date) {
			...
			$get_day = strtotime($attendance_date);
			$day = date('l', $get_day);
			$absen = $this->Timesheet_model->data_absen($employee_id, $attendance_date);
			$data_absen = array();
			if ($absen == TRUE) {
				$clock_in = new DateTime($absen[0]->clock_in);
				$clock_in2 = $clock_in->format('h:i a');

				$clock_out = new DateTime($absen[0]->clock_out);
				$clock_out2 = $clock_out->format('h:i a');
				$status = $absen[0]->attendance_status;
				$shift = $absen[0]->shift_name;
			} else {
				$clock_in2 = "00:00";
				$clock_out2 = "00:00";
				$status = "No data";
			...
			//    attendance date
			$id = $this->Timesheet_model->getupdate($employee_id, $attendance_date);
			$tdate = $this->Xin_model->set_date_format($attendance_date);
			$telat = $this->Xin_model->hitungtelatabsen($employee_id, $attendance_date);
			$pulang = $this->Xin_model->hitungpulangawal($employee_id, $attendance_date);
			$lembur_awal = $this->Xin_model->lembur_awal($employee_id, $attendance_date);
			$lembur = $this->Xin_model->hitunglembur($employee_id, $attendance_date);
			$lembur_holiday = $this->Xin_model->lemburholidays($employee_id, $attendance_date);
	...
	public function update_attendance_add() {
		...
		$data = array(
			...
			'all_shift' => $this->db->get('xin_office_shift'),
		);
		$session = $this->session->userdata('username');
		if (!empty($session)) {
	...
	public function add_leave() {
		...
		if ($this->input->post('add_type') == 'leave') {
			...
			$data = array(
				...
				'remarks' => $qt_remarks,
				'total_days' => $this->Timesheet_model->totalcuti($this->input->post('start_date'), $this->input->post('end_date')),
				'status' => '1',
				'created_at' => date('Y-m-d h:i:s'),
			);
	...
	public function add_attendance() {
		...
		if ($this->input->post('add_type') == 'attendance') {
			...
			$data = array(
				...
				'attendance_date' => $attendance_date,
				'code' => $this->input->post('shift_id', TRUE),
				'clock_in' => $clock_in2,
				'clock_out' => $clock_out2,
				'time_late' => $clock_in2,
				'total_work' => $total_work,
				...
				'attendance_status' => $this->input->post('status', true),
				'clock_in_out' => '0',
			);
			$result = $this->Timesheet_model->add_employee_attendance($data);
