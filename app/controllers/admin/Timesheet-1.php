...
class Timesheet extends MY_Controller {
					...
					if (is_uploaded_file($_FILES['file']['tmp_name'])) {
						...
						if (filesize($_FILES['file']['tmp_name']) > 51200000) {
							...
							foreach ($sheetData as $key) {
								...
								// office shift
								// $key = $this->Timesheet_model->shift_import($id_shift,$attendance_date);
								//     if ($key[0]->monday_in_time == '' && $day == 'Monday') {
								//        $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->tuesday_in_time == '' && $day == 'Tuesday') {
								//          $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->wednesday_in_time == '' && $day == 'Wednesday') {
								//         $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->thursday_in_time == '' && $day == 'Thursday') {
								//         $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->friday_in_time == '' && $day == 'Friday') {
								//         $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->saturday_in_time == '' && $day == 'Saturday') {
								//         $status = $this->lang->line('xin_holiday');
								//     } else if ($key[0]->sunday_in_time == '' && $day == 'Sunday') {
								//         $status = $this->lang->line('xin_holiday');
								//     } else{
								//         $status="Hadir";
								//     }
								//$ceksift=$this->Timesheet_model->cekshift($user[0]->user_id,$clock_in,$clock_out);
								$data = array(
							...
							//     while(($line = $sheetData) !== FALSE){

							//         $attendance_date = $line['B'];
							//         $clock_in = $line['C'];
							//         $clock_out = $line['D'];
							//         $clock_in2 = $attendance_date.' '.$clock_in;
							//         $clock_out2 = $attendance_date.' '.$clock_out;
							...
							//             $status='Alpa';
							//             }elseif ($clock_out==NULL) {
							//                 $status='Tidak Absen Pulang';
							//             }elseif ($clock_in==NULL) {
							//             $status='Tidak Absen Datang';
							//             }else{
							//                 $status='Hadir';
							//             }
	...
	public function updateabsen() {
		...
		foreach ($date_range as $date) {
			...
			// office shift
			$office_shift = $this->Timesheet_model->shift_employe($employee[0]->office_shift_id);
			...
			// }
			//    attendance date
			$id = $this->Timesheet_model->getupdate($employee[0]->employee_id, $attendance_date);
			$tdate = $this->Xin_model->set_date_format($attendance_date);
			$jam = $out_time - $in_time;
			$total = ($out_time < $jam) ? $jam - $out_time : 0;
			$data[] = array(
				$id,
				$tdate,
				$clock_in2,
