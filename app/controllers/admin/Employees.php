...
class Employees extends MY_Controller {
	...
	public function index() {
		...
		$data['contact'] = $this->Xin_model->contract();
	...
	public function employees_list() {
		...
		foreach ($employee->result() as $r) {
			...
			// get status
			if ($r->is_active == 0): $status = "<label class='alert alert-danger'>" . $this->lang->line('xin_employees_inactive') . "</label>";
			elseif ($r->is_active == 1): $status = "<label class='alert alert-success'>" . $this->lang->line('xin_employees_active') . "</label>";endif;
			// user role
			...
			if ($r->user_id == '1') {
				...
				$option = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_view_details') . '"><a href="' . site_url() . 'admin/employees/detail/' . $r->user_id . '"><button type="button" class="btn icon-btn btn-xs btn-outline-info waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span><span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_delete') . '"><button type="button" class="btn icon-btn btn-xs btn-outline-danger delete" data-toggle="modal" data-target=".delete-modal" data-record-id="' . $r->user_id . '"><span class="far fa-trash-alt"></span></button></span>';
	...
	public function detail() {
		...
		$data = array(
			...
			'id_absensi' => $result[0]->id_absensi,
			...
			'all_type' => $this->db->get('xin_contract_type')->result(),
		);
	...
	public function dialog_shift() {
		...
		$data = array(
			...
			'shift_all' => $this->db->get('xin_office_shift')->result(),
		);
		if (!empty($session)) {
			$this->load->view('admin/employees/dialog_employee_details', $data);
		} else {
	...
	public function add_employee() {
		...
		if ($this->input->post('add_type') == 'employee') {
			...
			} else if (strlen($this->input->post('password')) < 3) {
			...
			$id_absensi = $this->Xin_model->clean_post($this->input->post('id_absensi'));
			...
			$data = array(
				...
				'contract_type_id' => $this->Xin_model->clean_post($this->input->post('contract')),
				'address' => $address,
				'is_active' => 1,
	...
	public function shift_info() {
		...
		if ($this->input->post('type') == 'shift_info') {
			...
			$start_date = new DateTime($this->input->post('from_date'));
			...
			foreach ($date_range as $date) {
				...
				$data = array(
					...
					'employee_id' => $this->input->post('user_id'),
					'total_days' => $this->Timesheet_model->hitunghari($this->input->post('from_date'), $this->input->post('to_date')),
					'status' => 1,
					'bulan' => substr($this->input->post('from_date'), 0, 7),
					'created_at' => date('d-m-Y'),
				);
	...
	public function shift() {
		...
		foreach ($shift->result() as $r) {
			...
			$data[] = array(
				'<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_edit') . '"><button type="button" class="btn btn-secondary btn-sm m-b-0-0" data-toggle="modal" data-target=".edit-modal-data" data-field_id="' . $r->emp_shift_id . '" data-field_type="shift"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_delete') . '"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="' . $r->emp_shift_id . '" data-token_type="shift"><i class="fa fa-trash-o"></i></button></span>',
				$this->Xin_model->set_date_format($r->shift_date),
				$shift_name . " [" . $shift_info[0]->monday_in_time . "] - " . "[" . $shift_info[0]->monday_out_time . "]",
						...
						if (filesize($_FILES['file']['tmp_name']) > 2000000) {
							...
							while (($line = fgetcsv($csvFile)) !== FALSE) {
								...
								$data = array(
									...
									'office_shift_id' => 1,
									'is_active' => $line[13],
									'contract_type_id' => $line[11],
									'first_name' => $line[0],
									'last_name' => $line[1],
									'username' => $line[2],
									'email' => $line[3],
									'password' => password_hash($line[4], PASSWORD_BCRYPT, $options),
									'id_absensi' => $line[5],
