...
class Xin_model extends CI_Model {
	...
	public function caricuti($a, $b) {
		$sql = 'SELECT sum(view_sisa_cuti.total_days) as total_cuti,view_sisa_cuti.* from view_sisa_cuti where (view_sisa_cuti.from_date between view_sisa_cuti.from_date and view_sisa_cuti.to_date) and view_sisa_cuti.employee_id = ? AND view_sisa_cuti.status=2 and view_sisa_cuti.from_date LIKE ?';
		$binds = array($a, $b . "%");
	...
	public function tidakabsen($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as telat FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Tidak Absen Datang'
    OR xin_attendance_time.attendance_status='Tidak Absen Pulang'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	}
	public function telat($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as total_telat FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Terlambat'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row();
	...
	public function pulang($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as total_pulang FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Pulang Awal'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row();
	...
	public function getsakit($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as total_sakit FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Sakit'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function getijin($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as total_ijin FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Ijin'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function ijinkhusus($a, $b) {
		$sql = "SELECT count(xin_attendance_time.time_attendance_id) as total_khusus FROM xin_attendance_time
	WHERE xin_attendance_time.attendance_date LIKE ? AND xin_attendance_time.employee_id=? AND  xin_attendance_time.attendance_status='Ijin Khusus'";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function getalpa($a, $b) {
		$sql = "SELECT count(time_attendance_id) as total_alpa FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND  attendance_status='Alpa' AND code!=' '";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function gettelatdatang($em_id, $date, $jadwal, $import) {
		$sql = "SELECT attendance_date,employee_id,time_attendance_id,
  TIMEDIFF(TIME('$import'),'$jadwal') AS selisih,
  HOUR(TIMEDIFF(TIME('$import'),'$jadwal')) AS jam_telat,
  MINUTE(TIMEDIFF(TIME('$import'),'$jadwal')) AS menit_telat,
  SECOND(TIMEDIFF(TIME('$import'),'$jadwal')) AS detik_telat,
  attendance_status FROM  xin_attendance_time WHERE employee_id=? AND attendance_date LIKE ?";

		$binds = array($em_id, $date . "%");
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row();
		} else {
	...
	public function getpulangcepat($em_id, $date, $jadwal, $absensi) {
		$sql = "SELECT attendance_date,employee_id,time_attendance_id,
  TIMEDIFF(TIME('$import'),'$absensi') AS selisih,
  HOUR(TIMEDIFF(TIME('$import'),'$absensi')) AS jam_telat,
  MINUTE(TIMEDIFF(TIME('$import'),'$absensi')) AS menit_telat,
  SECOND(TIMEDIFF(TIME('$import'),'$absensi')) AS detik_telat,
  attendance_status FROM  xin_attendance_time WHERE employee_id=? AND attendance_date LIKE ?";
	...
	public function pulangcepat($em_id, $date, $jadwal, $import) {
		$sql = "SELECT attendance_date,employee_id,time_attendance_id,
  TIMEDIFF(TIME('$jadwal'),'$import') AS selisih,
  HOUR(TIMEDIFF(TIME('$jadwal'),'$import')) AS jam_cepat,
  MINUTE(TIMEDIFF(TIME('$jadwal'),'$import')) AS menit_cepat,
  SECOND(TIMEDIFF(TIME('$jadwal'),'$import')) AS detik_cepat,
  attendance_status FROM  xin_attendance_time WHERE employee_id=? AND attendance_date LIKE ?";
	...
	public function hitungtelat() {
		//kebalik
		$sql = 'SELECT attendance_date,employee_id,time_attendance_id,clock_in,monday_in_time,
  TIMEDIFF(TIME(clock_in),monday_in_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_in),monday_in_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_in),monday_in_time)) AS menit,
  SECOND(TIMEDIFF(TIME(clock_in),monday_in_time)) AS detik,
  attendance_status
FROM
  view_absensi WHERE employee_id=1 AND attendance_date LIKE "2019-03%" AND attendance_status !="Libur" AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti" AND TIMEDIFF(TIME(clock_in),monday_in_time)>0';
	...
	public function hitungcepat() {
		$sql = 'SELECT attendance_date,employee_id,time_attendance_id,clock_out,monday_out_time,
  TIMEDIFF(TIME(clock_out),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_out),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_out),monday_out_time)) AS menit,
  SECOND(TIMEDIFF(TIME(clock_out),monday_out_time)) AS detik,
  attendance_status
FROM
  view_absensi WHERE employee_id=1 AND attendance_date LIKE "2019-03%" AND attendance_status !="Libur" AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti" and TIMEDIFF(TIME(clock_out),monday_out_time)<0';
	...
	function hitungot_otomatis() {
		$sql = 'SELECT monday_in_time,monday_out_time,attendance_date,employee_id,time_attendance_id,clock_out,monday_out_time,
  TIMEDIFF(TIME(clock_out),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_out),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_out),monday_out_time)) AS menit,
  SECOND(TIMEDIFF(TIME(clock_out),monday_out_time)) AS detik,
  attendance_status
FROM
  view_absensi WHERE employee_id=1 AND attendance_date LIKE "2019-02%" AND attendance_status="Overtime Holidays" AND TIMEDIFF(TIME(clock_out),monday_out_time)>0';

		$query = $this->db->query($sql);
		return $query->result();
	}
	function hitunglemburpayroll($a, $b) {
		$sql = 'SELECT
  TIMEDIFF(TIME(clock_out),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_out),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_out),monday_out_time)) AS menit,
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status ="Overtime"  AND TIMEDIFF(TIME(clock_out),monday_out_time)>0';
		...
		if ($query->num_rows() > 0) {
			...
			return $total_lembur9;
		} else {
			return 0;
		}

	}
	function lembur10($a, $b) {
		$sql = 'SELECT
  TIMEDIFF(TIME(clock_out),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_out),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_out),monday_out_time)) AS menit,
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status ="Overtime"  AND TIMEDIFF(TIME(clock_out),monday_out_time)>0 group by attendance_date';
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($query->result() as $key1) {
				...
				$total_lembur10 += $ot_10;
			}
			return $total_lembur10;
		} else {
			return 0;
		}
	}
	function lemburx8() {
		$sql = 'SELECT clock_in,clock_out, monday_in_time,monday_out_time,
  TIMEDIFF(TIME(monday_in_time),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(monday_in_time),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(monday_in_time),monday_out_time)) AS menit,
  SECOND(TIMEDIFF(TIME(monday_in_time),monday_out_time)) AS detik,
  attendance_status
FROM
  view_absensi WHERE employee_id=1 AND attendance_date LIKE "2019-02%" AND attendance_status ="Overtime Holidays" and TIMEDIFF(TIME(monday_in_time),monday_out_time)<0 ';
		$query = $this->db->query($sql);

		return $query->result();

	}
	public function lemburholiday8($a, $b) {
		$sql = 'SELECT sum(HOUR(total_work))as jam , sum(MINUTE(total_work))as menit,employee_id,attendance_date,attendance_status from view_absensi WHERE attendance_status="overtime Holidays" and employee_id=? AND attendance_date LIKE ?  GROUP BY attendance_date';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
		$total = 0;
		$lembur = 0;
		foreach ($query->result() as $key) {
			$jam = $key->jam - 1;
			$menit = $key->menit;
			if ($jam == 8 and $menit == 0) {
				$lembur = 8;
			} elseif ($jam == 8 and $menit < 31) {
				$lembur = 8 + 0.5;
			} else {
				$lembur = 0;
			}
	...
	function lemburholiday9($a, $b) {
		...
		foreach ($query->result() as $key) {
			$jam = $key->jam - 1;
			$menit = $key->menit;
			if ($jam == 9 and $menit == 0) {
				$lembur = 1;
			} elseif ($jam == 9 and $menit < 31) {
				$lembur = 1 + 0.5;
			} else {
				$lembur = 0;
			}
	...
	public function lemburholiday10($a, $b) {
		$sql = 'SELECT count(attendance_date)as total , sum(MINUTE(total_work))as menit,employee_id,attendance_date,attendance_status from view_absensi WHERE attendance_status="overtime Holidays" and employee_id=?  AND attendance_date LIKE ? AND HOUR(total_work)>=11 GROUP BY attendance_date';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
		$total = 0;
		$lembur = 0;
		foreach ($query->result() as $key) {
			...
			$menit = $key->menit;
			if ($total_count > 0 and $menit == 0) {
				$lembur = $total_count;
			} elseif ($total_count > 0 and $menit < 31) {
				$lembur = $total_count + 0.5;
			} elseif ($total_count > 0 AND $menit > 31) {
				$lembur = $total_count + 1;
			} else {
				$lembur = 0;
			}
			$total += $lembur;
...
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status ="Overtime" AND TIMEDIFF(TIME(clock_out),monday_out_time)>0';
	...
	function lembur_awal($a, $b) {
		$sql = 'SELECT attendance_date,employee_id,time_attendance_id,clock_in,monday_in_time, TIMEDIFF(TIME(clock_in),monday_in_time) AS selisih, HOUR(TIMEDIFF(TIME(clock_in),monday_in_time)) AS jam, MINUTE(TIMEDIFF(TIME(clock_in),monday_in_time)) AS menit, SECOND(TIMEDIFF(TIME(clock_in),monday_in_time)) AS detik, attendance_status FROM view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status="Overtime" AND TIMEDIFF(TIME(clock_in),monday_in_time)<0';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
		$total = 0;
		$lembur = 0;
		if ($query->num_rows() > 0) {
			...
			foreach ($query->result() as $key1) {
				...
				// 			$lembur = $key1->jam;
				// 		}
				// 		// if ($key1->jam > 0 and $key1->menit > 48) {
				// 		// 	$lembur = $key1->jam + 0.5;
				// 		// }
				// 		if ($key1->jam > 0 and $key1->menit > 50) {
				// 			$lembur = $key1->jam + 1;
				// 		}
				// 	} elseif ($key1->jam > 0 and $key1->menit > 48) {
				// 		$lembur = $key1->jam;
				// 	}
				// 	// if ($key1->jam == 0 and $key1->menit <= 31) {
				// 	// 	$lembur = 0.5;
	...
	function lemburholidays($a, $b) {
		$sql = 'SELECT * FROM view_absensi WHERE employee_id = ? and attendance_date = ? and total_work != "" and attendance_status="Overtime Holidays"';
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		...
		$total_time_rs = '';
		$hrs_old_int_res1 = '';
		foreach ($query->result() as $hour_work) {
			// total work
	...
	function lemburholidaypayroll($a, $b) {

	}
	public function hitungtelatpayrol($a, $b) {
		$sql = 'SELECT attendance_date,employee_id,time_attendance_id,clock_in,monday_in_time,
  TIMEDIFF(TIME(clock_in),monday_in_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_in),monday_in_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_in),monday_in_time)) AS menit,
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti" and TIMEDIFF(TIME(clock_in),monday_in_time)>0';
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti" AND attendance_status!="Overtime Holidays" AND TIMEDIFF(TIME(clock_in),monday_in_time)>0';
	...
	public function hitungpulangpayrol($a, $b) {
		$sql = 'SELECT attendance_date,employee_id,time_attendance_id,clock_out,monday_out_time,
  TIMEDIFF(TIME(clock_out),monday_out_time) AS selisih,
  HOUR(TIMEDIFF(TIME(clock_out),monday_out_time)) AS jam,
  MINUTE(TIMEDIFF(TIME(clock_out),monday_out_time)) AS menit,
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti" AND TIMEDIFF(TIME(clock_out),monday_out_time)<0';
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($query->result() as $key1) {
				...
				if ($key1->jam == 0 and $key1->menit == 0 AND $key1->selisih = 0) {
					$cepet = 0;
				} else {
					if ($key1->jam > 0 and $key1->menit > 0 AND $key1->selisih < 0) {
						if ($key1->jam > 0 and $key1->menit == 0 AND $key1->selisih < 0) {
						...
						if ($key1->jam > 0 and $key1->menit < 31 AND $key1->selisih < 0) {
							$cepet = $key1->jam + 0.5;
						}
						if ($key1->jam > 0 and $key1->menit > 31 AND $key1->selisih < 0) {
							$cepet = $key1->jam + 1;
						}
					} elseif ($key1->jam > 0 and $key1->menit == 0 AND $key1->selisih < 0) {
						$cepet = $key1->jam;
					} elseif ($key1->jam == 0 and $key1->menit > 31 AND $key1->selisih < 0) {
						$cepet = 1;
					}
					if ($key1->jam == 0 and $key1->menit <= 31 AND $key1->selisih < 0) {
						$cepet = 0.5;
  ...
  attendance_status
FROM
  view_absensi WHERE employee_id=? AND attendance_date LIKE ? AND attendance_status !="Libur" AND attendance_status!="Alpa" AND attendance_status!="cuti"AND attendance_status!="Overtime Holidays" AND TIMEDIFF(TIME(clock_out),monday_out_time)<0 ';
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($query->result() as $key1) {
				if ($key1->jam == 0 and $key1->menit == 0) {
					...
					if ($key1->jam == 0 and $key1->menit < 3) {
						$cepet = 0;
					}
				}
				$total += $cepet;
	...
	public function tanggalshift($a, $b) {
		$sql = "SELECT * FROM view_shift WHERE shift_date=? AND employee_id=?";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row()->shift_date;

		} else {
			return "NOT SET";
		}
	}
	function shiftsore($a, $b) {
		$sql = 'SELECT COUNT(time_attendance_id) as total_sore,attendance_status
from view_absensi WHERE (employee_id=?  AND attendance_date LIKE ?) AND (monday_in_time LIKE "13:00" OR monday_in_time LIKE "14:00" OR monday_in_time LIKE "15:00") ';
	...
	function shiftmalam($a, $b) {
		$sql = 'SELECT COUNT(time_attendance_id) as total_malam,attendance_status
from view_absensi WHERE (employee_id=?  AND attendance_date LIKE ?) AND (monday_in_time LIKE "20:00" OR monday_in_time LIKE "21:00" OR monday_in_time LIKE "22:00") ';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
	...
	public function ambilin($a, $b) {
		$sql = "SELECT * FROM view_shift WHERE shift_date=? AND employee_id=?";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row()->monday_in_time;
	...
	public function ambilshift($a, $b) {
		$sql = "SELECT * FROM xin_employee_shift WHERE employee_id= ? AND shift_date= ?";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
	...
	public function ambilout($a, $b) {
		$sql = "SELECT * FROM view_shift WHERE shift_date=? AND employee_id=?";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row()->monday_out_time;

		} else {
			return "NOT SET";
		}
	}
	public function ot9($a, $b) {
		$sql = "SELECT sum(lembur) as ot9 FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND attendance_status='Overtime'
    AND lembur =1";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function ot10($a, $b) {
		$sql = "SELECT sum(lembur) as ot10 FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND attendance_status='Overtime'
    AND lembur >1";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function oth8($a, $b) {
		$sql = "SELECT sum(lembur) as oth8 FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND attendance_status='Overtime Holidays'
    AND lembur =8";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function oth9($a, $b) {
		$sql = "SELECT sum(lembur) as oth9 FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND attendance_status='Overtime Holidays'
    AND lembur =9";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function oth10($a, $b) {
		$sql = "SELECT sum(lembur) as oth10 FROM view_absensi
	WHERE attendance_date LIKE ? AND employee_id=? AND attendance_status='Overtime Holidays'
    AND lembur >9";
		$binds = array($a . '%', $b);
		$query = $this->db->query($sql, $binds);
	...
	public function getjabatan($id) {
		$sql = 'SELECT xin_employees.* ,xin_designations.designation_name FROM xin_employees INNER JOIN xin_designations ON(xin_designations.designation_id=xin_employees.designation_id)  WHERE xin_employees.user_id = ? ';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function getmasukkerja($a, $b) {

		$sql = 'SELECT COUNT(view_absensi.time_attendance_id) as total_hadir,view_absensi.*
FROM view_absensi WHERE (view_absensi.employee_id= ?  AND view_absensi.attendance_date LIKE ?) AND (view_absensi.attendance_status="Hadir" OR view_absensi.attendance_status="Pulang Awal" OR view_absensi.attendance_status="Terlambat" OR view_absensi.attendance_status="Overtime" or view_absensi.attendance_status="Overtime Holidays")';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
	...
	public function totalharikerja($a, $b) {
		$sql = 'SELECT count(view_absensi.time_attendance_id) as total_hari FROM view_absensi WHERE view_absensi.employee_id=? AND view_absensi.attendance_date LIKE ? AND view_absensi.attendance_status="Hadir" OR view_absensi.attendance_status="Terlambat" OR view_absensi.attendance_status="Pulang Awal"';
	...
	public function hitungjumlah($a) {

		$sql = 'SELECT xin_make_payment.* ,sum(`basic_salary`)+sum(`medical_allowance`)+SUM(`travelling_allowance`)+sum(`house_rent_allowance`)+sum(`overtime_rate`)
as total_pendapatan FROM xin_make_payment WHERE employee_id=? GROUP BY make_payment_id ';
		$binds = array($a);
		$query = $this->db->query($sql, $binds);
	...
	public function totalpengurangan($a) {
		$sql = 'SELECT `provident_fund`+`tax_deduction`+`security_deposit` as total_potongan from xin_make_payment
 WHERE employee_id=? GROUP BY make_payment_id ';
		$binds = array($a);
		$query = $this->db->query($sql, $binds);
	...
	public function cekcuti($a, $b) {
		$sql = 'SELECT * FROM view_sisa_cuti where employee_id = ? AND from_date like  ? AND status=? ';
		$binds = array($a, $b . '%', 2);
		$query = $this->db->query($sql, $binds);
	...
	public function getcuti($a, $b) {

		$sql = 'SELECT count(xin_leave_applications.leave_id) as total_cuti,xin_leave_applications.* from xin_leave_applications where (xin_leave_applications.from_date between xin_leave_applications.from_date and xin_leave_applications.to_date) and xin_leave_applications.employee_id = ? AND xin_leave_applications.status=2 and xin_leave_applications.from_date LIKE ?';
		$binds = array($a, $b . "%");
		$query = $this->db->query($sql, $binds);
	...
	public function read_user_by_employee_id($id) {
		...
		$sql = 'SELECT * FROM xin_employees WHERE id_absensi = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
