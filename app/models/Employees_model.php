...
class employees_model extends CI_Model {
	...
	public function get_employees() {
		$this->db->select('E.*,T.name as type');
		$this->db->from('xin_employees AS E');
		$this->db->join('xin_contract_type AS T', 'E.contract_type_id = T.contract_type_id', 'left');
		$this->db->where('E.is_active', 1);
		return $this->db->get();
	}
	function shift($id) {
		$sql = "SELECT * FROM view_absensi WHERE employee_id= ?";
		$binds = array($id);
	...
	public function get_employees1() {
		$this->db->select('E.*,T.name as type');
		$this->db->from('xin_employees AS E');
		$this->db->join('xin_contract_type AS T', 'E.contract_type_id = T.contract_type_id', 'left');
		return $this->db->get();
	...
	public function get_total_employees() {
		$query = $this->db->get_where("xin_employees", ['is_active' => 1]);
