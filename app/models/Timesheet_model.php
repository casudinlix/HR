...
class Timesheet_model extends CI_Model {
	...
	function cek_libur($date) {
		$sql = 'SELECT * FROM xin_holidays WHERE start_date LIKE ? limit 1';
		$binds = array($date);
		$query = $this->db->query($sql, $binds);
	...
	function ambil_libur($a) {
		$sql = 'SELECT * FROM xin_holidays WHERE start_date LIKE ?';
		$binds = array($a);
		$query = $this->db->query($sql, $binds);
	...
	public function leave_date_check($emp_id, $attendance_date) {
		...
		$sql = 'SELECT * from xin_leave_applications where (from_date between from_date and to_date) and employee_id = ? and from_date = ? and to_date = ? limit 1';
		$binds = array($emp_id, $attendance_date, $attendance_date);
		$query = $this->db->query($sql, $binds);

		return $query;
	}
	function cek_cuti($id, $date) {
		$sql = 'SELECT * from view_sisa_cuti where (from_date between from_date and to_date) and employee_id = ? AND from_date = ? and to_date = ? AND status=2 limit 1';
		$binds = array($id, $date, $date);
		$query = $this->db->query($sql, $binds);
	...
	function cekcuti($emp_id, $attendance_date) {
		$sql = 'SELECT * from xin_leave_applications where (from_date between from_date and to_date) and employee_id = ? AND from_date = ?  limit 1';
	...
	public function sisacuti($a, $b) {
		$sql = 'SELECT * FROM view_sisa_cuti where leave_type_id = ? AND employee_id = ? AND status=? ';
		$binds = array($a, $b, 2);
		$query = $this->db->query($sql, $binds);
	...
	public function totalcuti($a, $b) {
		$from = new DateTime($a);
		$to = new DateTime($b);
		return $total = $from->diff($to)->d + 1;

	}
	public function hitunghari($a, $b) {
		$from = new DateTime($a);
		$to = new DateTime($b);
		return $total = $from->diff($to)->d + 1;

	}
	public function cekbeda($a, $b) {
		$from = new DateTime($a);
		$to = new DateTime($b);
		return $total = $from->diff($to)->h;
	}
	...
	public function getupdate($a, $b) {
		$sql = 'SELECT * FROM view_absensi where employee_id = ? and attendance_date = ?';
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $key) {
				return $functions = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_edit') . '"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light edit-data" data-toggle="modal" data-target=".edit-modal-data" data-attendance_id="' . $key->time_attendance_id . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_delete') . '"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="' . $key->time_attendance_id . '"><i class="fa fa-trash-o"></i></button></span>';
			}
	...
	public function shift_employe($id) {
		$sql = "SELECT * FROM view_shift WHERE office_shift_id= ?";
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->result();
	...
	public function get_shift_employee($a, $b) {
		$sql = "SELECT * FROM view_shift WHERE employee_id= ? AND shift_date =? ";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->result();
	...
	public function shift_import($a, $b) {
		$sql = "SELECT * FROM view_shift WHERE shift_id= ? AND shift_date =? ";
		$binds = array($a, $b);
		$query = $this->db->query($sql, $binds);
		$get_day = strtotime($b);
		$day = date('l', $get_day);
	...
	public function cekshift($a, $b, $c, $d) {
		$sql = 'SELECT * FROM xin_attendance_time WHERE employee_id = ? AND attendance_date= ? AND clock_in=? AND clock_out=? limit 2';
		$binds = array($a, $b, $c, $d);
		$query = $this->db->query($sql, $binds);
		if (count(['B']) == 2) {
			$in = ['C'];
