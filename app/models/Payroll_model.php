...
class payroll_model extends CI_Model {
	...
	public function totaljamkerja($a, $b) {

		$sql = 'SELECT sum(total_work) as total FROM xin_attendance_time WHERE employee_id = ? and attendance_date like ?';
		$binds = array($a, $b . '%');
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->row();
	...
	public function total_lemburholidays($a, $b) {
		$sql = 'SELECT sum(HOUR(total_work))as jam , sum(MINUTE(total_work))as menit,employee_id,attendance_date,attendance_status
from view_absensi WHERE attendance_status="overtime Holidays" AND employee_id=? AND attendance_date LIKE ?  GROUP BY attendance_date';
		$binds = array($a, $b . '%');
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	public function advance_salaries_report_view($id) {
		$sql = 'SELECT advance_salary_id,company_id,employee_id,month_year,one_time_deduct,monthly_installment,reason,status,total_paid,is_deducted_from_salary,created_at,SUM(`xin_advance_salaries`.advance_amount) AS advance_amount FROM `xin_advance_salaries` where status=1 and employee_id= ? group by advance_salary_id, employee_id';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		return $query->result();
	}
	...
	public function get_paid_salary_by_employee_id($id) {
		...
		$sql = 'SELECT advance_salary_id,employee_id,month_year,one_time_deduct,monthly_installment,reason,status,total_paid,is_deducted_from_salary,created_at,SUM(`xin_advance_salaries`.advance_amount) AS advance_amount FROM `xin_advance_salaries` where status=1 and employee_id=? group by employee_id,advance_salary_id';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		return $query->result();
	}
